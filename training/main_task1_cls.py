...
def train_epoch(model, dataloader, optimizer, criterion, device):
    ...
    for batch_x, batch_class in progress_bar:
        batch_x = batch_x.to(device, non_blocking=True)
        batch_class = batch_class.to(device, non_blocking=True)
        
        optimizer.zero_grad()
        class_logits = model(batch_x)
        loss = criterion(class_logits, batch_class)
        loss.backward()
        optimizer.step()
        
        running_loss += loss.item() * batch_x.size(0)
        _, predicted = torch.max(class_logits, 1)
        correct += (predicted == batch_class).sum().item()
        total += batch_class.size(0)
...
def train_model(model, train_loader, val_loader, optimizer, criterion,
    ...
    for epoch in range(num_epochs):
        ...
        best_model_path = checkpoint_dir / f'task1Cls_seed_{seed}_run_{run_index}_best.pt'
        last_model_path = checkpoint_dir / f'task1Cls_seed_{seed}_run_{run_index}_last.pt'
...
def main():
    parser = argparse.ArgumentParser(description="Train task1Cls with multi-seed runs")
    parser.add_argument(
        "--num_seeds",
        type=int,
        default=5,
    ...
    dataset = SpeciesDataset(h5_file)
    ...
    for i, seed in enumerate(random_seeds):
        ...
        checkpoint_dir = f'checkpoints_multiseed_species/seed_{i}'
    ...
    summary_dir = Path('checkpoints_multiseed_species')
