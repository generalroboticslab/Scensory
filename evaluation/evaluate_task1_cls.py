...
from src.model_task1_cls import SpeciesClassifier
from src.task1_components import SpeciesDataset
...
def load_model(checkpoint_path, device, model_config=None):
    ...
    try:
        ...
        model = SpeciesClassifier(**config).to(device)
...
def find_model_checkpoints(checkpoint_dir):
    ...
    patterns_to_try = [
        str(checkpoint_dir / "task1Cls.pt"),
        str(checkpoint_dir / "**/task1Cls_seed_*_run_*_best.pt"),
        str(checkpoint_dir / "best_species_*.pt")  # Fallback to any species classifier
    ]
...
def extract_seed_info(checkpoint_path):
    ...
    try:
        # Handle pattern: best_species_classifier_run0_seed12345.pt
...
def evaluate_model(model, dataloader, device):
    ...
    for batch_x, batch_class in dataloader:
        batch_x = batch_x.to(device, non_blocking=True)
        batch_class = batch_class.to(device, non_blocking=True)
        
        class_logits = model(batch_x)
        _, predicted = torch.max(class_logits, 1)
        
        all_preds.extend(predicted.cpu().numpy())
        all_labels.extend(batch_class.cpu().numpy())
...
def plot_box_metrics(metrics_data, class_names, save_dir):
    ...
    # Define color palette - extend for 5 species classes
    box_colors = [
        colors['primary'],      # Light Blue - Base
        colors['accent'],       # Light Orange/Amber - Accent
        colors['harmonious1'],  # Light Teal - Harmonious
        colors['harmonious2'],  # Light Purple - Harmonious
        colors['contrast']      # Slightly deeper Blue for 5th class
    ]
    ...
    axes[0].set_title('Precision by Species', **TITLE_FONT)
    axes[0].set_ylabel('Precision', **LABEL_FONT)
    axes[0].set_xlabel('Species', **LABEL_FONT)
    ...
    axes[1].set_xlabel('Species', **LABEL_FONT)
    ...
    axes[2].set_title('F1-Score by Species', **TITLE_FONT)
    axes[2].set_ylabel('F1-Score', **LABEL_FONT)
    axes[2].set_xlabel('Species', **LABEL_FONT)
    axes[2].set_xticks(range(len(class_names)))
    axes[2].set_xticklabels(class_names)
    axes[2].grid(True, alpha=0.3, axis='y', color=border_color)

    # Set y-limits for all plots to be consistent
    ...
    plt.tight_layout()
    plt.savefig(save_dir / 'per_class_metrics_boxplot_species.png', bbox_inches='tight', dpi=300)
    plt.close()
...
def save_comprehensive_results(all_results, results_dir, class_names):
    ...
    with open(results_dir / 'evaluation_summary_species.txt', 'w') as f:
        f.write("Multi-Seed Species Classification Evaluation Summary\n")
        f.write("=" * 60 + "\n\n")
    ...
    with open(results_dir / 'detailed_evaluation_results_species.json', 'w') as f:
...
def main():
    ...
    results_dir = Path('Results/task1_classification')
    ...
    test_dataset = SpeciesDataset(test_h5_file)
    test_loader = DataLoader(test_dataset, batch_size=32, shuffle=False, num_workers=4, pin_memory=True)

    # Class names for species classification
    ...
    for i, model_path in enumerate(checkpoint_files):
        ...
        if seed is not None:
            cm_path = results_dir / f'confusion_matrix_species_seed_{seed}_run_{run_id}.png'
            title_suffix = f' (Seed {seed}, Run {run_id})'
        else:
            cm_path = results_dir / f'confusion_matrix_species_model_{i+1}.png'
    ...
    if len(individual_results) > 1:
        ...
        plot_accuracy_comparison(individual_results, results_dir / 'accuracy_comparison_species.png')
    else:
    ...
    if len(all_results) > 1:
        ...
        plot_confusion_matrix(all_results[0]['y_true'], ensemble_preds, class_names,
                            results_dir / 'confusion_matrix_species_ensemble.png', 
